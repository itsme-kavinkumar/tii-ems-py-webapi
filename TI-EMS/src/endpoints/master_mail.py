...
async def save_mail_details(mail_id : int = Form(""),
    ...
    try:
        ...
        if send_date == '':
            send_date = '1900-01-01 00:00:00'
        
        if mail_id == "":            
            await save_mail(cnx,from_mail,to_mail,pwd,cc_mail,bcc_mail,subject,compose_textarea,report,mail_type,send_at,send_date,user_login_id,campus_id,mail_for)
            return _getSuccessResponseJson("Saved Successfully...")
        else:
            await update_mail(cnx,mail_id,from_mail,to_mail,pwd,cc_mail,bcc_mail,subject,compose_textarea,report,mail_type,send_at,send_date,user_login_id,campus_id,mail_for)
