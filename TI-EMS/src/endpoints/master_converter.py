...
async def save_converter_details(campus_id : str = Form(""),
                                 ...
                                 baud_rate : str =Form(''),                       
                                 parity : str =Form(''),                                 
                                 is_communication_check : str =Form(''),                                 
                                 cnx: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if converter_id == '':
            ...
            await save_converter(cnx,campus_id,converter_model_id,converter_name,ip_address,port_no,meter_limit,user_login_id,mac,company_id,bu_id,plant_id,baud_rate,parity,is_communication_check)
            createFolder("Log/","Query executed successfully for save plant converter")
            return _getSuccessResponseJson("Saved Successfully...")
        else:
            await update_converter(cnx,campus_id,converter_model_id,converter_id,converter_name,ip_address,port_no,meter_limit,user_login_id,mac,company_id,bu_id,plant_id,baud_rate,parity,is_communication_check)
