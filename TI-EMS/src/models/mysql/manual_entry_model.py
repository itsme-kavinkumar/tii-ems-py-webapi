...
async def savemanual_entry(cnx,obj,user_login_id,plant_id,request):
    ...
    for i in user_dict:
        ...
        if kwh_reset =='yes':
            ...
            sql = f'''insert into data_correction(mill_date,mill_shift,plant_id,is_manual_call,is_ckwh_correction)
                    values('{mill_date}','{mill_shift}','{plant_id}','yes','no')'''
...
async def manualdata_correction(cnx,mill_date,mill_shift,plant_id,is_ckwh_correction):
    mill_date = await parse_date(mill_date)
    sql = f'''insert into data_correction(mill_date,mill_shift,plant_id,is_manual_call,is_ckwh_correction)
              values('{mill_date}','{mill_shift}','{plant_id}','yes','no')'''
    createFolder("Manual_Entry_Log/", "data_correction.." + str(sql))
