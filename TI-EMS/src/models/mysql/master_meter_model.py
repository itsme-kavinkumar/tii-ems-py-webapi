...
async def meter_Lists(cnx, company_id,bu_id,meter_id,type_value,type_id,is_critical,model_name,plant_id,plant_department_id,function_id,function2_id,holiday,selected,campus_id,meter,equipment_id,equipment_group_id,meter_type,selected_meter,ip_address,source,is_main_transformer_meter):
        ...
        if campus_id !='' and campus_id != "0":
            ...
            where1 += f" and md.campus_id = {campus_id}" 
        ...
        if is_main_transformer_meter == 'yes':
            where += " and main_transformer_meter = 'yes'"
        ...
        query = text(f"""
                    SELECT mm.*,
                        ...
                        IFNULL(me.equipment_code,'') as equipment_code,
                        IFNULL(me.equipment_name,'') as equipment_name,
                        
                        IFNULL(mm.other_equipment_name,'')other_equipment_name,
...
async def save_meter(cnx, company_id,bu_id,plant_id,plant_department_id,campus_id,function_id,converter_id,meter_id,meter_code,meter_name,other_equipment_name,ip_address,port,major_nonmajor,model_name,IMEI,user_login_id,function2_id,address,parameter, sub_parameter,meter_type,source,physical_location,max_demand,max_pf,main_demand_meter,consumption_type,is_poll_meter,meter,equipment_group_id,equipment_id,mac):

        query = text(f"""
                INSERT INTO ems_v1.master_meter (
                    company_id,  bu_id, plant_id, plant_department_id,equipment_group_id,equipment_id,  meter_code, meter_name,other_equipment_name, address,
...
async def update_meter(cnx, company_id,bu_id,plant_id,plant_department_id,campus_id,function_id,converter_id,meter_id,meter_code,meter_name,other_equipment_name,ip_address,port,major_nonmajor,model_name,IMEI,user_login_id,function2_id,address,parameter, sub_parameter,meter_type,source,physical_location,max_demand,max_pf,main_demand_meter,consumption_type,is_poll_meter,meter,equipment_group_id,equipment_id,mac):
        ...
        query =text(f"""
                ...
                other_equipment_name = '{other_equipment_name}',
        ...
        await cnx.execute(query)
        await cnx.commit()
        if equipment_id  != '' and equipment_id is not None and meter == "equipment":
            qry= text(
                f"""

                    update ems_v1.master_equipment_calculations mec SET mec.equipment_id ={equipment_id} ,mec.meter_communication = '{meter}' WHERE mec.equipment_id={meter_id}
                """
            )
            print('*******qry',qry)
            await cnx.execute(qry)
            await cnx.commit()

        if equipment_id  != '' and equipment_id is not None and meter == "common": 
            qry= text(
                f"""

                    update ems_v1.master_equipment_calculations mec SET mec.equipment_id={meter_id} ,mec.meter_communication = '{meter}' WHERE  mec.equipment_id ={equipment_id}
                """
            )
            print('(_(___)_)',qry)
            await cnx.execute(qry)
...
async def transformer_submeter_list(cnx,campus_id,main_transformer_meter_id):
    ...
    query = f'''
            select  
                ...
                '' as main_transformer_meter_name,
    ...
    for row in data:
        ...
        for meter_id in meter_id_list:                             
            ...
            for sub_row in sub_data:
                ...
                meter_dtl += f'''{sub_row['meter_code']}-{sub_row['meter_name']}''' 
                print(meter_dtl)           
        trans_meter_name = text(f"select * from ems_v1.master_meter where meter_id = {main_transformer_meter_id}")    
        name_data = await cnx.execute(trans_meter_name)
        name_data = name_data.fetchall()    
        for rows in name_data:  
            meter_name = rows["meter_name"]  
        
        new_row = dict(row)
        new_row["meter_dtl"] = meter_dtl
        new_row["main_transformer_meter_name"] = meter_name
        result.append(new_row)         
    return result 
