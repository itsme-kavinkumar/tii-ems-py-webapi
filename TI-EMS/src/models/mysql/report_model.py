...
async def current_power_dtl(cnx,company_id ,campus_id,bu_id ,plant_id ,plant_department_id ,equipment_group_id ,equipment_id,function_id ,meter_id ,group_for ,groupby ,period_id,from_date,to_date,shift_id,limit_report_for,limit_exception_for,limit_order_by ,limit_operation_value ,is_critical ,converter_id ,report_for,is_function , function_type ,reportfor,employee_id ,is_minmax,is_main_meter,is_demand,meter_type,is_plant_wise):    
    ...
    if reportfor == '12to12':
        ...
        if period_id == "cur_shift":               
            ...
            min_kwh_tname = 'ems_v1.current_power'
            ...
            min_kwh_tname = f"  {completed_db}power_{month_year} "
                ...
                if from_date.month == to_date.month and from_date.year == to_date.year:
                    ...
                    for month_year in month_year_range:
                        ...
                        if len(result_query) > 0:
                            ...
                            min_kwh_tname_list.append(f"SELECT master_kWh,machine_kWh,power_id FROM {table_name}")
                            union_queries.append(f"SELECT {field_name} FROM {table_name}")
    ...
    if min_kwh_check == 1:
        min_kwh_tname = " UNION ALL ".join(min_kwh_tname_list)
        min_kwh_tname = f"({min_kwh_tname}) as p_ix"

    if period_id in ("cur_shift","#cur_shift","sel_shift","#previous_shift"):
        select_min_kwh = "ROUND(min(CASE WHEN cp.mill_shift = 1 THEN case when mmf.machine_kWh = '*' then cp.master_kwh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.master_kwh / mmf.machine_kWh_value else cp.master_kwh end  END),prf.machine_kwh) AS start_kwh_1,"
        select_max_kwh = "ROUND(max(CASE WHEN cp.mill_shift = 3 THEN case when mmf.machine_kWh = '*' then cp.machine_kWh * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then cp.machine_kWh / mmf.machine_kWh_value else cp.machine_kWh end  END),prf.machine_kwh) AS end_kwh_3,"
    else:
        select_min_kwh = f"ROUND((CASE WHEN mmf.machine_kWh = '*' THEN (SELECT master_kwh FROM {min_kwh_tname} WHERE power_id=MIN(cp.power_id)) * mmf.machine_kWh_value WHEN  mmf.machine_kWh = '/' THEN (SELECT master_kwh FROM {min_kwh_tname} WHERE power_id=MIN(cp.power_id)) / mmf.machine_kWh_value ELSE (SELECT master_kwh FROM {min_kwh_tname} WHERE power_id=MIN(cp.power_id)) END),prf.machine_kwh) AS start_kwh_1,"
        select_max_kwh = f"ROUND((case when mmf.machine_kWh = '*' then (SELECT machine_kWh FROM {min_kwh_tname} WHERE power_id=MAX(cp.power_id)) * mmf.machine_kWh_value when  mmf.machine_kWh = '/' then (SELECT machine_kWh FROM {min_kwh_tname} WHERE power_id=MAX(cp.power_id)) / mmf.machine_kWh_value else (SELECT machine_kWh FROM {min_kwh_tname} WHERE power_id=MAX(cp.power_id)) end),prf.machine_kwh) AS end_kwh_3,"
...
'''
    ...
    query = text(f'''
            ...
            ''')
    print('1231_query', query)
    createFolder("Log/user_countlog/","equipment query "+str(query))
...
async def month_report(cnx,campus_id,company_id,bu_id,plant_id,plant_department_id,equipment_group_id,employee_id,meter_id,month_year,report_for,report_type,meter_type,kwh,request):
        ...
        query = text(f'''
            ...
            FROM
                ...
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = md.plant_id                   
            WHERE
                mm.status = 'active'{where} and DATE_FORMAT(cp.mill_date ,'%m-%Y') = '{month_year}' 
            GROUP BY
                mm.meter_id,
                cp.mill_date
                {groupby} 
            order by md.plant_id,ms.plant_department_id,mm.meter_id              
        ''')
            ...
            for row in rslt:
                ...
                if meter_code not in output:
                    output[meter_code] = {
                        ...
                        "plant_department_name": plant_department_name,
...
async def hour_wise_analysis_report(cnx,campus_id,company_id,bu_id,plant_id,plant_department_id,equipment_group_id,employee_id,meter_id,from_date,meter_type,kwh,month_year,from_time,to_time,request,a_shift_start_time):
        ...
        query=text(f'''
            SELECT
                ...
                DATE_FORMAT(cp.created_on,'%Y-%m-%d %H') AS dt_hour,
                mm.meter_type,
                md.plant_name,
...
async def hour_min_wise_report(cnx,campus_id,company_id,bu_id,plant_id,plant_department_id,equipment_group_id,employee_id,meter_id,from_date,meter_type,kwh,month_year,from_time,to_time,report_for,a_shift_start_time): 
    where = ""
    rslt = ''
    kwh_type = kwh
    if meter_id == "" or meter_id == 'all':
    ...
    if a_shift_start_time != '':
        next_date = from_date + timedelta(days=1)
        next_date = next_date.strftime("%Y-%m-%d") 
        where +=f" and DATE_FORMAT(cp.created_on ,'%Y-%m-%dT%H:%i:%s')<'{next_date}T{a_shift_start_time}:00'"
    select = ''
    group_by = ''
    if report_for == 'hour_wise':
        select = f'''CASE
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 0 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 15 THEN 00
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 15 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 30 THEN 15
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 30 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 45 THEN 30
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 45 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 60 THEN 45
            ELSE -1
        END AS dt_min,'''
        group_by = f"GROUP BY meter_id,dt_hour"
    elif report_for == '5min':
        select = f'''CASE
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 0 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 5 THEN 00
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 5 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 10 THEN 05
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 10 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 15 THEN 10
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 15 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 20 THEN 15
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 20 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 25 THEN 20
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 25 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 30 THEN 25
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 30 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 35 THEN 30
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 35 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 40 THEN 35
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 40 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 45 THEN 40
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 45 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 50 THEN 45
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 50 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 55 THEN 50
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 55 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 59 THEN 55
            ELSE -1
        END AS dt_min,'''
        group_by = f"GROUP BY meter_id,dt_hour,dt_min"
    
    elif report_for == '10min':
        select = f'''CASE
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 0 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 10 THEN 00
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 10 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 20 THEN 10
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 20 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 30 THEN 20
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 30 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 40 THEN 30
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 40 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 50 THEN 40
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 50 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 59 THEN 50
            ELSE -1
        END AS dt_min,'''
        group_by = f"GROUP BY meter_id,dt_hour,dt_min"


    elif report_for == '15min':
        select = f'''CASE
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 0 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 15 THEN 00
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 15 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 30 THEN 15
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 30 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 45 THEN 30
    ...
    elif report_for == '20min':
        select = f'''CASE
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 0 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 20 THEN 00
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 20 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 30 THEN 20
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 40 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 59 THEN 40
            ELSE -1
        END AS dt_min,'''
        group_by = f"GROUP BY meter_id,dt_hour,dt_min"

    elif report_for == '30min':
        select = f'''CASE
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 0 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) < 30 THEN 00
            WHEN CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) >= 30 && CAST(DATE_FORMAT(cp.created_on,'%i') AS UNSIGNED) <= 59 THEN 30
            ELSE -1
        END AS dt_min,'''
        group_by = f"GROUP BY meter_id,dt_hour,dt_min"
    ...
    #         {where}
    #     {group_by}
    #     ORDER BY meter_id,dt_hour,dt_min'''
    ...
    query = f'''
        SELECT
            ...
            ROUND(
                    SUM(
                        CASE
                            WHEN mmf.machine_kWh = '*' THEN cp.diff_machine_kWh * mmf.machine_kWh_value
                            WHEN mmf.machine_kWh = '/' THEN cp.diff_machine_kWh / mmf.machine_kWh_value
                            ELSE cp.diff_machine_kWh
                        END
                    ),
                    prf.machine_kWh
                ) AS kwh,
    ...
    if report_for == 'hour_wise':
        ...
        if rslt !='':
            ...
            for entry in rslt:
                ...
                if (pre_meter_code != meter_code) or pre_machine_kwh == '':
                    master_kwh = master_kwh
                else:
                    master_kwh = pre_machine_kwh

                pre_meter_code = meter_code
                pre_machine_kwh = machine_kwh
                ...
                if kwh_type == 'Calculated':
                    ...
                    hourly_data[meter_code][hour_key] = machine_kwh-master_kwh
                hourly_data[meter_code][f"machine_kwh_{hour_key}"] = machine_kwh
                hourly_data[meter_code][f"master_kwh_{hour_key}"] = master_kwh
            hourly_data = list(hourly_data.values())
            for row in rslt:
        ...
        if len(rslt)>0:
            for row in rslt:
                ...
                output[meter_code].append({"meter_code":meter_code,
                                    "meter_name":meter_name,
                                    "machine_kwh":machine_kwh,
                                    "master_kwh":master_kwh,
                                    "kwh":machine_kwh - master_kwh,
                                    "dt_min":dt_min,
                                    "dt_hour":dt_hour,
                                    "meter_type":meter_type,
                                    "plant_name":plant_name,
...
async def demand_report(period_id,campus_id,meter_id,from_date,to_date,shift_id,from_time,to_time,filter_type,parameter,duration,main_demand_meter,date_time,end_time,cnx):
    ...
    if period_id == "cur_shift":               
        ...
        if from_date != '' and to_date != '':
            if from_date.month == to_date.month and from_date.year == to_date.year:
                ...
                field_name = 'mill_date,mill_shift,kva,actual_demand,t_watts,demand_dtm,avg_powerfactor,t_watts,created_on'
    ...
    if parameter == 'pf' and filter_type != 'all':
        ...
        query2 = '''
                    ...
                    SELECT meter_code,meter_name,meter_id,company_id,bu_id,plant_id,campus_id,campus_name,max_demand,mill_date,mill_shift,demand,actual_demand,d_date_time,dm_powerfactor,date_time,kw,kva
                    FROM RankedData
                    WHERE row_num = 1'''
...
async def availability_report(cnx,campus_id,from_date,to_date,year,report_type):
    ...
    subquery_union = " UNION ALL ".join(union_queries)
    table_name = f"( {subquery_union}) cp"
    year_month=f"""{str(from_date.year)}{mill_month[from_date.month]}"""  
    select = ''
    if to_date != '': 
        if report_type == 'with_rate':
            ...
            select = f'''(
                    SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
                    FROM master_despatch_month d1
                    WHERE d1.ORGANIZATION_CODE = REPLACE(SUBSTRING(SUBSTRING_INDEX(mp.plant_code, '-', 3), LENGTH(SUBSTRING_INDEX(mp.plant_code, '-', 3 - 1)) + 1), '-', '')
                    AND (d1.ITEM_CAT1 = mbe.reporting_department or d1.ITEM_CAT1 ='' ) and d1.DESPATCH_MONTH = '{year_month}'
                    AND d1.ITEM_CAT2 <> 'Other Plant'
            ) AS cust_wh_ton,
            (
                    SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
                    FROM master_despatch_month d1
            ...
            select = f'''(
                    SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
                    FROM master_despatch_day d1
                    WHERE d1.ORGANIZATION_CODE = REPLACE(SUBSTRING(SUBSTRING_INDEX(mp.plant_code, '-', 3), LENGTH(SUBSTRING_INDEX(mp.plant_code, '-', 3 - 1)) + 1), '-', '')
                    AND (d1.ITEM_CAT1 = mbe.reporting_department or d1.ITEM_CAT1 ='' )  and d1.DESPATCH_DATE >='{from_date}' and d1.DESPATCH_DATE <='{to_date}'
                    AND d1.ITEM_CAT2 <> 'Other Plant'
            ) AS cust_wh_ton,
            (
                    SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
    ...
    query2 = f'''
            ...
            where mm.status = 'active' and mbe.status = 'active' and mbe.financial_year = '{year}' {where1}
    ...
    query3 = f'''
            ...
            where mm.status = 'active' and mm.source = 'DG' and  mm.meter_type = 'primary' {where1}
    ...
    query4 = f'''
            ...
            from 
                ...
                left JOIN master_budget_rate mbr ON mbr.energy_source_name = mm.source 
                and mbr.campus_id = mp.campus_id and mbr.month = cp.mill_date and mbr.source_type = 'internal' 
            where mm.status = 'active' and mm.source not in  ('EB','DG') and  mm.meter_type = 'primary' {where1}
            group by mp.campus_id,mm.source
            order by mm.plant_id,mm.source
            '''
    createFolder("Log/", "availability report query4 " + str(query4))
    data4 = await cnx.execute(query4)
    data4 = data4.mappings().all()
    print("query4 : ", query4)
    if report_type == 'with_rate':
        query5 = f'''
                select
                    ...
                    ifnull(mbr.budget_demand,0) as budget_demand,
                    ifnull(mbr.actual_demand_final,0) as actual_demand_final,
                    ifnull(mbr.actual_demand,0) as actual_demand,
                    mse.campus_id,
                    mbr.month,
                    mse.energy_source_name
                    FROM
                        master_source_entry mse  
                        left join master_budget_rate mbr on mbr.energy_source_name = mse.energy_source_name and mbr.campus_id = mse.campus_id and mbr.source_type = mse.source_type and  mbr.month >='{from_date}'and  mbr.month <='{to_date}'
                    Where mse.status = 'active' AND mse.source_type = 'fixed_demand' and mse.campus_id = '{campus_id}'
                '''
        createFolder("Log/", "availability report query5 " + str(query5))
        data5 = await cnx.execute(query5)
        data5 = data5.mappings().all()
        print("query5 : ", query5)
        
        
# query for show debit and credit value :
        
        
        condtion_3 = ''
        if campus_id == '3':
            condtion_3 = 'reporting_department,'

        query_6 = f"""  SELECT *
                        FROM master_account_code_entry
                        WHERE STATUS != 'delete'
                            AND campus_id = {campus_id}
                        ORDER BY
                            {condtion_3}
                        FIND_IN_SET(unit_type,'variable,fixed,generation'), FIND_IN_SET(rate_type,'variable,fixed,generation,tax_percentage,rate_per_unit'), FIND_IN_SET(account_type, 'debit,credit')         
                """
        
        data_6 = await cnx.execute(query_6)
        data_6 = data_6.mappings().all()
        print("query_6 : ", query_6)
        
        
    if data != [] and data2!= []:
        result = {"data":data,"data2":data2,"data3":data3,"data4":data4,"data5":data5, "data_6":data_6}
    else:
        result = []
    print('result:',result)
    return result
...
async def transformerlossreportold(cnx,campus_id,from_date,to_date):
    where = ''
    if campus_id != '':
        where += f" and mp.campus_id ='{campus_id}' "
    if from_date != '' and to_date != '':
...
async def transformerlossreport(cnx,campus_id,from_date,to_date):
    ...
    sql = f''' select mm.meter_id,mm.meter_code,mm.meter_name,c.campus_name,mp.plant_name 
                from master_meter mm 
                inner join master_plant mp on mp.plant_id = mm.plant_id
                inner join master_campus c on c.campus_id = mp.campus_id 
                where mm.main_demand_meter = 'yes' {where1} order by mm.campus_id,mm.meter_id'''
    data = await cnx.execute(sql)
    data = data.fetchall()

    formatted_data_list = []
    output = {}
    formula_with_child_mt_id = ""
    inner_select = ""
    if len(data)>0:
        for row in data:
            meter_code = row["meter_code"]
            ...
            sql1 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{meter_id}]%'  and status = 'active'"
            data1 = await cnx.execute(sql1)
            data1 = data1.fetchall()
            
            if len(data1)>0:
                for row1 in data1:
                    formula2 = row1["formula2"]
        ...
        createFolder("Log/", "transformer_loss formatted_data_list : " + str(formatted_data_list))

        if formatted_data_list != []:
            for formatted_data in formatted_data_list:
                campus_name = formatted_data["campus_name"]
                formula = formatted_data["formula2"]
                meters = re.findall(r'\(([^)]+)\)', formula)
      
                if meters:
                    formula = f"({','.join(meters)})"
                    meters = re.findall(r'\[(\d+)\]', formula)
                    formula2 = f"{','.join(meters)}"
                else:
                    meters = re.findall(r'\[(\d+)\]', formula)
                    meters = meters[1:]  # Eliminate the first meter
                    formula2 = f"{','.join(meters)}"

                empty_data_mt_id = []
                case_condition = ''
                list_numbers = [int(number) for number in formula2.split(',')]
                for mt_id in list_numbers:
                    sql1 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{mt_id}]%'  and status = 'active'"
                    data1 = await cnx.execute(sql1)
                    data1 = data1.fetchall()
                    
                    if data1 :

                        matches = re.findall( r"dict\[(\d+)\]", data1[0]['formula2'].split('-')[0]) 
                        result =f"{','.join(matches)}"

                        case_condition+= f' WHEN mm.meter_id IN ({result}) THEN {mt_id} '
                        empty_data_mt_id.append(mt_id)
                        formula_with_child_mt_id += f'{result},'
                        
                        # child_meter_id[mt_id] = result.replace(f'{mt_id},', '')
                        # result = [ int(data) for data in result.split(',')]
                        # for data in result:
                        #     formula_with_child_mt_id += f'{data},'

                createFolder("Log/", "transformer_loss formula2 : " + str(formula2))
                
                if len(formula2)>0:
                    formula_with_child_mt_id = formula_with_child_mt_id[:-1]                     
                    numbers = ','.join(map(str, empty_data_mt_id))
                    print(numbers)
                   
                    if empty_data_mt_id:
                        inner_select=f"""ROUND(SUM(
                                                CASE
                                                    WHEN mm.meter_id IN ({numbers}) THEN cp.equipment_kwh 
                                                    ELSE 0
                                                END
                                            ), prf.kWh) AS calculated_kwh,"""
                    else:
                        inner_select=f"ROUND(SUM(cp.equipment_kwh),prf.kWh) AS calculated_kwh,"
                ...
                sql3 = f'''
                        select 
                            ...
                            mm.meter_name,
                            CONCAT((SELECT 
                                        CONCAT(c_mm.meter_code, '-', c_mm.meter_name) 
                                    FROM 
                                        master_meter c_mm 
                                    WHERE 
                                        c_mm.meter_id = CASE
                                                            {case_condition} 
                                                        END), ' - ', mp.plant_name) AS meter_code,
                            c.campus_name,
                            mp.campus_id,
                            mp.plant_name,
                            ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end ),prf.kWh) AS kWh,                          
                        ...
                        from 
                            ...
                            inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                            inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                        where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' {where} and mm.meter_id  in ({formula_with_child_mt_id}) 
                        group by
                            CASE
                                {case_condition} 
                            END,
                ...
                createFolder("Log/", "transformer_loss query sql3" + str(sql3))
                rslt = await cnx.execute(sql3)
                rslt = rslt.fetchall()

                print('rslt',rslt)
                createFolder("Log/", "transformer_loss query rslt" + str(rslt))
                
                # for i in list_numbers:
                #     query = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{i}]%'  and status = 'active'"
                #     data_find_cal = await cnx.execute(query)
                #     data_find_cal = data_find_cal.fetchall()
                ...
                if len(rslt) >0:
                    ...
                    for row in rslt:
                        ...
                        output[meter_code]["total_kwh"] += kwh
                        output[meter_code][f'cal_kwh_d{day}'] = calculated_kwh
                        output[meter_code]["total_cal"] += calculated_kwh   # here to change...
    ...
    createFolder("Log/", "transformer_loss result" + str(result))
...
async def submeterlossreport_old(cnx,campus_id,main_transformer_meter_id,from_date,to_date):
    where = ''
    if campus_id != '':
        where += f" and mp.campus_id ='{campus_id}' "
...
async def submeterlossreport(cnx,campus_id,from_date,to_date):
    where = ''
    where1 = ''
    rslt = ''
    result = ''
    ...
    if formatted_data_list!= []:
        for formatted_data in formatted_data_list:
            ...
            for meter in meters:
                main_meter_id = meter
                sql2 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{main_meter_id}]%'  and status = 'active'"
                data2 = await cnx.execute(sql2)
                data2 = data2.fetchall()
                if len(data2)>0:
                    for row1 in data2:
                        main_meter_formula = row1["formula2"]
                    sql_meter = f"select CONCAT(meter_code , '-' , meter_name) AS meter_name from master_meter where meter_id = {main_meter_id}"
                    meter_data = await cnx.execute(sql_meter)
                    meter_data = meter_data.fetchone()
                    main_meter_name = meter_data["meter_name"]
                    print("main_meter_name",main_meter_name)
                    meter_data = {
                    "main_meter_id": main_meter_id,
                    "main_meter_name":main_meter_name,
                    "plant_name": plant_name,
                    "campus_name": campus_name,
                    "main_meter_formula": main_meter_formula
                    }

                    main_meter_data.append(meter_data)
        print("main_meter_data",main_meter_data)
        if main_meter_data != []:

            for datas in main_meter_data:
                campus_name = datas["campus_name"]
                main_meter_name = datas["main_meter_name"]
                main_meter_id = datas["main_meter_id"]
                main_meter_formula = datas["main_meter_formula"]
                meters = re.findall(r'\(([^)]+)\)', main_meter_formula)
                if meters:
                    formula = f"({','.join(meters)})"
                    meters = re.findall(r'\[(\d+)\]', formula)
                    formula2 = f"{','.join(meters)}"
                else:
                    meters = re.findall(r'\[(\d+)\]', main_meter_formula)
                    meters = meters[1:]  # Eliminate the first meter
                    formula2 = f"{','.join(meters)}"
                ...
                if len(formula2)>0:
                    sql = f'''
                        ...
                        from 
                            ...
                            inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                            inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                        where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' and mm.meter_id in ({formula2}) {where}
                        group by mm.meter_id, cp.mill_date
                        order by mm.plant_id,mm.meter_id,cp.mill_date
                            '''
                    rslt = await cnx.execute(sql)
                    ...
                    for row in rslt:
                        ...
                        if meter_code not in output:
                            output[meter_code] = {
                                ...
                                "campus_name": campus_name,
                                "plant_name": plant_name,   
                                "main_transformer_meter_id": main_meter_id ,
                                "main_meter_name":main_meter_name   
                            }
...
async def campuswisebudgetreport(cnx,from_date,campus_id):
    where = ''
    if campus_id!= '' and campus_id != 0:
        where += f" and br.campus_id = {campus_id}"
    sql = f'''
            SELECT c.campus_name,br.*,
            case when br.energy_source_name = 'EB' or br.energy_source_name = 'TNEB' then 'TNEB/EB' else br.energy_source_name end  as energy_source_name 
            FROM master_budget_rate br
            INNER JOIN master_campus c ON c.campus_id = br.campus_id
            WHERE  MONTH = '{from_date}' and br.source_type = 'Internal' {where} ORDER BY c.campus_id,br.source_type,br.energy_source_name
    '''
    rslt = await cnx.execute(sql)
    rslt = rslt.fetchall()
    Internal = {}
    for row in rslt:
        ...
        if campus_name not in Internal:
            Internal[campus_name] = {}

        if budget != 0 and budget != "" :
            if energy_source_name not in Internal[campus_name]:
                Internal[campus_name][energy_source_name] = {
    ...
    sql = f'''
            ...
            FROM master_budget_rate br
            INNER JOIN master_campus c ON c.campus_id = br.campus_id
            WHERE  MONTH = '{from_date}' and br.source_type = 'External'{where} ORDER BY c.campus_id,br.source_type,br.energy_source_name
    '''
    rslt2 = await cnx.execute(sql)
    rslt2 = rslt2.fetchall()
    # print(rslt2)
    External = {}
    for row in rslt2:
        campus_name = row["campus_name"]
        ...
        budget = row["budget"]
        budget_mix = row["budget_mix"]
        # print("budget_mix",budget_mix)

        
        if campus_name not in External:
            External[campus_name] = {}
        if budget != 0 and budget != "" :
            if energy_source_name not in External[campus_name]:
                External[campus_name][energy_source_name] = {
                    'energy_source_name': energy_source_name,
                    'source_type': source_type,
                    'campus_name': campus_name,
                    'budget': budget,
    ...
    output = {"Internal":Internal,"External":External}
    return output

async def stagewisekwhvariation_report(cnx,campus_id,from_date,to_date):
    If_calculation = False
    where = ''
    if campus_id != '':
        where += f" and mp.campus_id ='{campus_id}' "
    if from_date != '' and to_date != '':
        ...
        table_name = f"( {subquery_union}) cp"
    dg_meter_kWh = 0
    main_demand_meter_data = []  
    final_main_transformer_data = []  
    final_sub_transformer_data = []
    final_sub_transformer_formula = []
    all_parent_meters1 = ''
    all_child_meters1 = ''
    parent_meter_data = []
    child_meter_data = []
    ...
    sql = f'''select 
                ...
                ROUND(SUM(cp.equipment_kwh),prf.kWh) AS calculated_kwh,
                (SELECT  SUM(msed.consumption) AS eb_entry FROM  master_source_entry_date msed
                INNER JOIN master_source_entry mse ON mse.id = msed.energy_source_id AND mse.energy_source_name = 'EB' AND mse.source_type = 'external' 
                WHERE  msed.mill_date >='{from_date}' AND msed.mill_date <='{to_date}'  AND msed.campus_id = {campus_id}) as eb_entry,
                case when mm.main_demand_meter = 'yes' then mm.meter_code end as main_demand_meter_code,
                (SELECT 
                    ROUND(SUM(CASE WHEN mmf_sub.kWh = '*' THEN scp.kWh * mmf_sub.kWh_value WHEN mmf_sub.kWh = '/' THEN scp.kWh / mmf_sub.kWh_value ELSE scp.kWh END), prf_sub.kWh)
                FROM 
                    ems_v1_completed.power_{month_year} scp
                JOIN master_meter sm ON sm.meter_id = scp.meter_id
                INNER JOIN master_meter_factor mmf_sub ON mmf_sub.plant_id = sm.plant_id AND mmf_sub.meter_id = sm.meter_id
                INNER JOIN master_parameter_roundoff prf_sub ON prf_sub.plant_id = sm.plant_id
                WHERE sm.status = 'active'
                AND scp.mill_date >= '{from_date}'
                AND scp.mill_date <='{to_date}' 
                AND sm.source NOT IN ('EB', 'DG')
                AND sm.meter_type = 'Primary'
                AND sm.campus_id = mp.campus_id) AS solar_kwh
            ...
            from 
                ...
                inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
            where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' and  mm.source IN ( 'DG')  AND mm.meter_type ='Primary' {where}
            group by mp.campus_id
            order by mm.plant_id,mm.meter_id
        '''
    data = await cnx.execute(sql)
    ...
    for row in data:
        ...
        plant_name = row["plant_name"]
        kWh = row["kWh"]
        DG += kWh if kWh is not None else 0
        solar_kwh = row['solar_kwh']  if row['solar_kwh'] is not None else 0
        calculated_kwh = row["calculated_kwh"]
        main_demand_meter_code = row["main_demand_meter_code"]

        if (row["eb_entry"] and row["kWh"])  is not None:
            result = row["eb_entry"] + row["kWh"] 
        else:
            result = row["eb_entry"]  if row["eb_entry"]  is not None else 0
        ...
        data = {
            ...
            "eb_entry": result + solar_kwh,
            "main_demand_meter_code": main_demand_meter_code,
        }
    ...
    sql1 = f'''select 
                ...
                mp.campus_id,
                mp.plant_name,
                mm.meter_code as main_demand_meter_code,
                mm.meter_name as main_demand_meter_name,
                ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end ),prf.kWh) AS kWh,
                ROUND(SUM(cp.equipment_kwh),prf.kWh) AS calculated_kwh
            from 
                {table_name}
                ...
                inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
            where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}'    and mm.main_demand_meter = 'yes' {where}
    ...
    if len(data1)>0:
        for row in data1:
            main_demand_meter_id = row["meter_id"]
            main_demand_meter_code = row["main_demand_meter_code"]
            main_demand_meter_name = row["main_demand_meter_name"]
            main_demand_meter_kwh = row["kWh"] + DG + solar_kwh
            main_demand_meter_cal_kwh = row["calculated_kwh"]
            query1 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{main_demand_meter_id}]%'  and status = 'active'"
            # print(query1)
            formula_data1 = await cnx.execute(query1)
            formula_data1 = formula_data1.fetchall()
           
            if len(formula_data1)>0:
                for row1 in formula_data1:
                    formula2 = row1["formula2"]
                    formula1 = row1["formula1"]
                
                formatted_data = {
                    "main_demand_meter_name": main_demand_meter_name,
                    "main_demand_meter_code": main_demand_meter_code,
                    "formula2": formula2,
                    "formula1":formula1,
                    "main_demand_meter_kwh":main_demand_meter_kwh,
                    "main_demand_meter_cal_kwh":main_demand_meter_cal_kwh
                }
                
                main_demand_meter_data.append(formatted_data)

        if main_demand_meter_data != []:
            list_count = 0
            for demand_meter_data in main_demand_meter_data:

                formula = demand_meter_data["formula2"]
                meters = re.findall(r'\(([^)]+)\)', formula)
                ...
                if len(formula2)>0:
                    sql2 = f'''
                        select 
                            ...
                            mp.campus_id,
                            mp.plant_name,
                            COUNT(DISTINCT mm.meter_id) AS meter_count,
                            ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value when  mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end ),prf.kWh) AS kWh,
                            ROUND(SUM(cp.equipment_kwh),prf.kWh) AS calculated_kwh
                        from 
                            {table_name}
                            ...
                            inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                            inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                        where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' {where}  and mm.meter_id  in ({formula2})
                        group by mm.meter_id
                        order by mm.plant_id,mm.meter_id
                            '''
                    data2 = await cnx.execute(sql2)
                    ...
                    for trans_m in data2:
                        main_transformer_meter_id = trans_m["meter_id"]
                        main_transformer_meter_code = trans_m["meter_code"]
                        main_transformer_meter_name = trans_m["meter_name"]
                        calculated_kwh = trans_m["calculated_kwh"]
                        main_transformer_meter_count = trans_m["meter_count"]
                        kWh = trans_m["kWh"]

                        query2 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{main_transformer_meter_id}]%' and formula2 LIKE '%-%'  and status = 'active'"
                        # print(query2)
                        formula_data2 = await cnx.execute(query2)
                        formula_data2 = formula_data2.fetchall()
                        
                        if len(formula_data2)>0:
                            for row1 in formula_data2:
                                transformer_formula2 = row1["formula2"]
                                formula1 = row1["formula1"]
                            list_count +=1
                        else:
                            transformer_formula2, formula1 = '', ''
                            

                        formatted_data2 = {
                            "main_transformer_meter_id": main_transformer_meter_id,
                            "main_transformer_meter_code": main_transformer_meter_code,#?---------------------------------
                            "main_transformer_meter_count":main_transformer_meter_count,
                            "main_transformer_meter_name":main_transformer_meter_name,
                            "transformer_meter_kWh": kWh,
                            "count": list_count,
                            "transformer_meter_cal_kwh": calculated_kwh,
                            "formula2":transformer_formula2,
                            "formula1":formula1
                        }
                
                        final_main_transformer_data.append(formatted_data2)

        if final_main_transformer_data != []:
            for transformer_d in final_main_transformer_data:
                main_transformer_meter_id = transformer_d["main_transformer_meter_id"]
                main_transformer_meter_code = transformer_d["main_transformer_meter_code"]
                formulas = transformer_d["formula2"]
                meters = ''
                all_meters = ''
                inner_meters = ''
                meters = re.findall(r'\(([^)]+)\)', formulas)
                # print(meters) 
                meters = re.findall(r'-\(([^)]+)\)', formulas)
                if len(meters) == 0 :
                    meters = re.findall(r'-(.*)', formulas)
                # print('metersmeters',meters)
                ...
                if meters :
                    ...
                    all_meters = re.findall(r'dict\[(\d+)\]', formulas)
                    inner_meters = re.findall(r"dict\[(\d+)\](?=[^\(\)]*\))", formulas)
                    if len(inner_meters) == 0 :
                        inner_meters = re.findall(r'-(.*)', formulas)
                        inner_meters = f"({','.join(inner_meters)})"
                        inner_meters = re.findall(r'\[(\d+)\]', inner_meters)
                        inner_meters = f"{','.join(inner_meters)}"
                    # print('inner_meters',inner_meters)
                    
                    dg_meter = [
                        meter for meter in all_meters if meter not in inner_meters
                    ]

                    if dg_meter:
                        del dg_meter[0]

                    formula3 = f"{','.join(dg_meter)}"
                ...
                if len(formula3) > 0:
                    sql3_dg = f"""
                        ...
                        from 
                            ...
                            inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                            inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                        where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' {where}  AND mm.meter_type ='Primary' and mm.meter_id  in ({formula3})
                        group by mp.campus_id
                        order by mm.plant_id,mm.meter_id
                            """   #  and mm.source in ('solar','DG') AND mm.meter_type ='Primary'
                    
                    data3_dg = await cnx.execute(sql3_dg)
                    data3_dg = data3_dg.fetchall()
                    # print(sql3_dg)
                    # print(data3_dg)
                    if len(data3_dg) > 0:
                        for dg_row2 in data3_dg:
                            transformer_d["transformer_meter_kWh"] += dg_row2["kWh"] 
                            print("dg_meter@#@_kWh...",dg_row2["kWh"])
                            dg_meter_kWh += dg_row2["kWh"]
                if len(formulas)>0:
                    sql3 = f'''
                        select 
                            mm.meter_id,
                    ...
                    data3 = await cnx.execute(sql3)
                    data3 = data3.fetchall()
                   
                    if len(data3)>0:
                        for s_row2 in data3:
                            sub_transformer_meter_id = s_row2["meter_id"]
                            sub_transformer_meter_code = s_row2["meter_code"]
                            sub_transformer_meter_kWh = s_row2["kWh"]
                            sub_transformer_meter_cal_kwh = s_row2["calculated_kwh"]
                            sub_transformer_meter_count = s_row2["meter_count"]
                        formatted_data3 = {
                                "main_transformer_meter_id":main_transformer_meter_id,
                                "main_transformer_meter_code":main_transformer_meter_code,
                                "sub_transformer_meter_id": sub_transformer_meter_id,
                                "sub_transformer_meter_code": sub_transformer_meter_code,
                                "sub_transformer_meter_count":sub_transformer_meter_count,
                                "sub_transformer_meter_kWh": sub_transformer_meter_kWh,
                                "sub_transformer_meter_cal_kwh": sub_transformer_meter_cal_kwh,
                                "sub_transformer_meters":formula2
                            }
                    
                        final_sub_transformer_data.append(formatted_data3)
                        all_meters = [] #!********
                        all_meters1 = ''
                        If_calculation = False
                        
                        for meter_id in formula2.split(','):
                            query3 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{meter_id}]%' AND formula2 LIKE '%-%'  and status = 'active'"
                            formula_data3 = await cnx.execute(query3)
                            formula_data3 = formula_data3.fetchall()
                            
                            if len(formula_data3)>0:
                                for row3 in formula_data3:
                                    sub_transformer_formula = row3["formula2"]
                                meters = re.findall(r'\(([^)]+)\)', sub_transformer_formula)
                                # print(meters) 
                                meters = re.findall(r'-\(([^)]+)\)', sub_transformer_formula)
                                if len(meters) == 0 :
                                    meters = re.findall(r'-(.*)', sub_transformer_formula)
                                ...
                                if meters :
                                    formula = f"({','.join(meters)})"
                                    meters = re.findall(r'\[(\d+)\]', formula)
                                    all_meters = f"{','.join(meters)}"
                                else:
                                    
                                    meters = re.findall(r'\[(\d+)\]', sub_transformer_formula)
                                    meters = meters[1:]  
                                    all_meters = f"{','.join(meters)}"
                                If_calculation = True
                            else:
                            ...
                            if all_meters != []:
                                all_meters1 +=  all_meters + ","
                         
                        if len(all_meters1)>0 and If_calculation:
                            sql4 = f'''
                                select 
                                    mm.meter_id,
                                    mm.meter_name,
                                ...
                                from 
                                    ...
                                    inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                                    inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                                where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' {where} and mm.meter_id  in ({all_meters1[:-1]})
                                order by mm.plant_id,mm.meter_id
                                    '''

                            data4 = await cnx.execute(sql4)
                            data4 = data4.fetchall()
                            if len(data4)>0:
                                for row4 in data4:
                                    parent_meter_id = row4["meter_id"]
                                    parent_meter_code = row4["meter_code"]
                                    parent_meter_kWh = row4["kWh"]
                                    parent_meter_cal_kwh = row4["calculated_kwh"]
                                    parent_meter_count = row4["meter_count"]
                                all_meters1 = all_meters1[:-1]
                                formatted_data4 = {
                                        "main_transformer_meter_id":main_transformer_meter_id,
                                        "main_transformer_meter_code":main_transformer_meter_code,
                                        "parent_meter_id": parent_meter_id,
                                        "parent_meter_code": parent_meter_code,
                                        "parent_meter_count":parent_meter_count,
                                        "parent_meter_kWh": parent_meter_kWh,
                                        "parent_meter_cal_kwh": parent_meter_cal_kwh,
                                        "parent_meters": all_meters1
                                    }
                            
                                parent_meter_data.append(formatted_data4)
                            all_parent_meters = []
                            all_parent_meters1 = ''
                            If_calculation = False
                            if all_meters1 != []:
                                for  parent_meter_id in all_meters1.split(','):
                                    query4 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{parent_meter_id}]%' AND formula2 LIKE '%-%'  and status = 'active'"
                                    formula_data4 = await cnx.execute(query4)
                                    formula_data4 = formula_data4.fetchall()
                                    if len(formula_data4)>0:
                                        for p_row4 in formula_data4:
                                            parent_formula = p_row4["formula2"]
            
                                        if parent_formula!= '':
                                            meters = re.findall(r'\(([^)]+)\)', parent_formula)
                                            # print(meters) 
                                            meters = re.findall(r'-\(([^)]+)\)', parent_formula)
                                            if len(meters) == 0 :
                                                meters = re.findall(r'-(.*)', parent_formula)
                                            ...
                                            if meters :
                                                formula = f"({','.join(meters)})"
                                                meters = re.findall(r'\[(\d+)\]', formula)
                                                all_parent_meters = f"{','.join(meters)}"
                                            else:
                                                
                                                meters = re.findall(r'\[(\d+)\]', parent_formula)
                                                meters = meters[1:]  
                                                all_parent_meters = f"{','.join(meters)}"
                                        If_calculation = True
                                    else:
                                        all_parent_meters= parent_meter_id
                                    if all_parent_meters!= []:
                                        all_parent_meters1 +=all_parent_meters+','
                            if len(all_parent_meters1)>0 and If_calculation:
                                sql5 = f'''
                                    select 
                                        mm.meter_id,
                                        mm.meter_name,
                                    ...
                                    from 
                                        ...
                                        inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                                        inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                                    where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' {where} and mm.meter_id  in ({all_parent_meters1[:-1]})
                                    order by mm.plant_id,mm.meter_id
                                        '''

                                data5 = await cnx.execute(sql5)
                                data5 = data5.fetchall()
                                if len(data5)>0:
                                    for row5 in data5:
                                        child_meter_id = row5["meter_id"]
                                        child_meter_code = row5["meter_code"]
                                        child_meter_kWh = row5["kWh"]
                                        child_meter_cal_kwh = row5["calculated_kwh"]
                                        child_meter_count = row5["meter_count"]
                                    all_parent_meters1 = all_parent_meters1[:-1]
                                    formatted_data5 = {
                                        "main_transformer_meter_id":main_transformer_meter_id,
                                        "main_transformer_meter_code":main_transformer_meter_code,
                                        "child_meter_id": child_meter_id,
                                        "child_meter_code": child_meter_code,
                                        "child_meter_count":child_meter_count,
                                        "child_meter_kWh": child_meter_kWh,
                                        "child_meter_cal_kwh": child_meter_cal_kwh,
                                        "child_meters":all_parent_meters1
                                    }
                            
                                    child_meter_data.append(formatted_data5)
                        all_child_meters = []
                        all_child_meters1 = ''
                        If_calculation = False
                        if all_parent_meters1!= []:
                            for  child_meter_id in all_parent_meters1.split(','):
                                query5 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{child_meter_id}]%' AND formula2 LIKE '%-%'  and status = 'active'"
                                formula_data5 = await cnx.execute(query5)
                                formula_data5 = formula_data5.fetchall()
                                if len(formula_data5)>0:
                                    for p_row5 in formula_data5:
                                        child_formula = p_row5["formula2"]
                                        meters = re.findall(r'\(([^)]+)\)', child_formula)
                                        # print(meters) 
                                        meters = re.findall(r'-\(([^)]+)\)', child_formula)
                                        if len(meters) == 0 :
                                            meters = re.findall(r'-(.*)', child_formula)
                                        ...
                                        if meters :
                                            formula = f"({','.join(meters)})"
                                            meters = re.findall(r'\[(\d+)\]', formula)
                                            all_child_meters = f"{','.join(meters)}"
                                        else:
                                            
                                            meters = re.findall(r'\[(\d+)\]', child_formula)
                                            meters = meters[1:]  
                                            all_child_meters = f"{','.join(meters)}"
                                    If_calculation = True
                                else:
                                    all_child_meters= child_meter_id
                                if all_child_meters != []:
                                    all_child_meters1 +=all_child_meters+','
                        if len(all_child_meters1)>0 and If_calculation:
                            sql5 = f'''
                            select 
                                mm.meter_id,
                                mm.meter_name,
                            ...
                            from 
                                ...
                                inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
                            where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' {where} and mm.meter_id  in ({all_child_meters1[:-1]})
                            order by mm.plant_id,mm.meter_id
                                '''

                            data6 = await cnx.execute(sql5)
                            data6 = data6.fetchall()
                            if len(data6)>0:
                                for row6 in data6:
                                    s_child_meter_id = row6["meter_id"]
                                    s_child_meter_code = row6["meter_code"]
                                    s_child_meter_kWh = row6["kWh"]
                                    s_child_meter_cal_kwh = row6["calculated_kwh"]
                                    s_child_meter_count = row6["meter_count"]
                                all_child_meters1 = all_child_meters1[:-1]   
                                formatted_data6 = {
                                    "main_transformer_meter_id":main_transformer_meter_id,
                                    "main_transformer_meter_code":main_transformer_meter_code,
                                    "s_child_meter_id":s_child_meter_id,
                                    "s_child_meter_code":s_child_meter_code,
                                    "s_child_meter_kWh":s_child_meter_kWh,
                                    "s_child_meter_cal_kwh":s_child_meter_cal_kwh,
                                    "s_child_meter_count":s_child_meter_count,
                                    "child_meters":all_child_meters1
                                }
                        
                                child_meter_data1.append(formatted_data6)
     
       
        return {"data":final_data,"data1":main_demand_meter_data,"data2":final_main_transformer_data,"data3":final_sub_transformer_data,"data4":parent_meter_data,"data5":child_meter_data,"data6":child_meter_data1,"dg_meter_kWh":dg_meter_kWh}
    else:
        return data

async def campuswisepowerconsumptionunits(cnx,campus_id,report_method,from_date,to_date,from_year,to_year):
    fin_from_date = from_date
    fin_campus_id = campus_id
    
    
    month_data = {}
    result_data = await GenerateReport(cnx, fin_campus_id, report_method, from_date, to_date, from_year, to_year)
    print('result_dataresult_data',result_data)
        
    for campusname, campusdata in result_data.items():
        if campusname not in month_data:
            month_data[campusname] = {
                "total_consumptions": 0,
                "total_budget_amount": 0,
                "total_actual_amount": 0,
                "total_rate_variation": 0,
                "total_total_amount": 0,
                "specific_consumption_variation": 0,
            }
        if campusdata['total_actual_amount'] != 0:
            month_data[campusname]['total_consumptions'] += campusdata['total_consumptions']
            month_data[campusname]['total_budget_amount'] += campusdata['total_budget_amount']
            month_data[campusname]['total_actual_amount'] += campusdata['total_actual_amount']
            month_data[campusname]['total_rate_variation'] += campusdata['total_rate_variation']
            month_data[campusname]['total_total_amount'] += campusdata['total_total_amount']
            month_data[campusname]['specific_consumption_variation'] += campusdata['specific_consumption_variation']

    fin_date = await fin_year((fin_from_date))  # To Get Financial year Data
    fin_year_data = {}
    for data in fin_date:
        from_date = datetime.strptime(data['startDate'], '%d-%m-%Y')
        to_date = datetime.strptime(data['endDate'], '%d-%m-%Y')
        result_data = await GenerateReport(cnx, fin_campus_id, report_method, from_date, to_date, from_year, to_year)
        
        for campusname, campusdata in result_data.items():
            if campusname not in fin_year_data:
                fin_year_data[campusname] = {
                    "total_consumptions": 0,
                    "total_budget_amount": 0,
                    "total_actual_amount": 0,
                    "total_rate_variation": 0,
            ...
            if campusdata['total_actual_amount'] != 0:
                fin_year_data[campusname]['total_consumptions'] += campusdata['total_consumptions']
                fin_year_data[campusname]['total_budget_amount'] += campusdata['total_budget_amount']
                fin_year_data[campusname]['total_actual_amount'] += campusdata['total_actual_amount']
                fin_year_data[campusname]['total_rate_variation'] += campusdata['total_rate_variation']
                fin_year_data[campusname]['total_total_amount'] += campusdata['total_total_amount']
                fin_year_data[campusname]['specific_consumption_variation'] += campusdata['specific_consumption_variation']
    
    result = {"data":month_data, "data2":fin_year_data}
    
    return result


# to get month and  financial year YTD  report
async def GenerateReport(cnx, campus_id, report_method, from_date, to_date, from_year, to_year):
    ...
    start_date = datetime(from_year, 4, 1)
    ...
    if from_year != '':
        where_year += f" and msed.mill_date >='{start_date}'"
        where_year_internal += f" and cp.mill_date >='{start_date}'"
    
    if to_year != '':
        where_year += f" and msed.mill_date <='{to_date}'"
        where_year_internal += f" and cp.mill_date <='{to_date}'"
     
    while current_date <= end_date:
        table_rows.append(current_date.strftime("%Y-%m-%d"))
        month_year = current_date.strftime("%m%Y")
        result_query = await check_power_table(cnx, month_year)
        today_date = datetime.today().date()

        if today_date.month == current_date.month and today_date.year == current_date.year:
            current_month_end_date = (datetime(today_date.year, today_date.month, 1) + relativedelta(months=1) - relativedelta(days=1)).date()

            if today_date >= current_month_end_date:

                if len(result_query) > 0:
                    table_name_year = f"ems_v1_completed.power_{month_year}"
                    union_query_year.append(f"SELECT {field_name} FROM {table_name_year}") 
                    year_month=f"""{str(from_date.year)}{mill_month[from_date.month]}""" 
                    year_months.append(f"{year_month}")
    ...
    if union_query_year != []:
        subquery_union_year = " UNION ALL ".join(union_query_year)
        table_name_year = f"( {subquery_union_year}) cp"
        year_month_all = ",".join(year_months)
    
    where_internal = ''
    where_external = ''

    if campus_id != '':
        where_external += f" and mse.campus_id = {campus_id}"
        where_internal += f" and mp.campus_id = {campus_id}"

    query = f'''
            select
                c.campus_name,
                ifnull(SUM(case when mse.energy_source_name = 'EB' then msed.consumption else 0 end ),0) AS provision_total_consumption,
                ifnull(sum(msed.consumption),0) as consumption,
                ifnull(SUM(case when mse.energy_source_name = 'EB' then msed.consumption_total else 0 end ),0) AS final_total_consumption,
                ifnull(sum(msed.consumption_total),0) as consumption_total,
                ifnull(mbr.budget,0) budget,
                ifnull(mbr.actual,0) actual,
                ifnull(mbr.actual_total,0) actual_total,
            ...
            FROM 
                ...
                left JOIN master_budget_rate mbr ON mbr.energy_source_name = mse.energy_source_name 
                and mbr.campus_id = mse.campus_id and mbr.month = msed.mill_date and mbr.source_type = 'external'
                INNER JOIN  master_campus c ON c.campus_id = mse.campus_id
            where 1=1  and mse.status ='active' and mse.source_type = 'external'{where_external}
            group by mse.id
            order by c.campus_id,mse.id
            '''
    createFolder("Log/", " report query " + str(query))
    data = await cnx.execute(query)
    data = data.mappings().all()
    output1 = {}
    ...
    if len(data) > 0:
        for row in data:
            ...
            if report_method == 'final':
                ...
                total_consumption = row["final_total_consumption"]
                actual = row["actual_total"]
            else:
                consumption = row["consumption"]
                total_consumption = row["provision_total_consumption"]
                actual = row["actual"]
            ...
            if campus_name not in output1:
                output1[campus_name] = []
                external_total_consumptions = 0
                overall_total = 0
            external_total_consumptions += total_consumption

            if energy_source_name != 'EB': 
                overall_total += consumption
                if energy_source_name == 'EB-METER RENT/CUSTOMER CHARGES':  
                    consumption = 1   
                output1[campus_name].append({
                    "consumption": consumption,
                    "budget_amount": consumption * budget,
                    "actual_amount": consumption * actual,
                    "rate_variation": (consumption * budget) - (consumption * actual),
                    "budget": budget,
                    "actual": actual,
                    "budget_mix": budget_mix,
                    "external_total_consumptions":external_total_consumptions,
    ...
    if len(data)>0:
        for row in data:
            ...
            if energy_source_name == 'EB':
                if campus_name in output1  and  len(output1[campus_name]) > 0:
                    try:
                        output1[campus_name].append({
                            "consumption": consumption - output1[campus_name][-1]["overall_total"],
                            "budget_amount": (consumption - output1[campus_name][-1]["overall_total"]) * budget,
                            "actual_amount": (consumption - output1[campus_name][-1]["overall_total"]) * actual,
                            "rate_variation": ((consumption - output1[campus_name][-1]["overall_total"]) * budget) - ((consumption - output1[campus_name][-1]["overall_total"]) * actual),
                            "budget": budget,
                            "actual": actual,
                            "budget_mix": budget_mix,
                            "external_total_consumptions": total_consumption
                        ...
                        createFolder("Log/", "Error in EB Budget calulation " + str(e))
                else:
                    output1[campus_name].append({
                        "consumption": consumption,
                        "budget_amount": consumption * budget,
                        ...
                        "actual": actual,
                        "budget_mix": budget_mix,
                        "external_total_consumptions":consumption
                    })
    ...
    query3 = f'''
            ...
            from 
                ...
                left JOIN master_budget_rate mbr ON mbr.energy_source_name = mm.source 
                and mbr.campus_id = mp.campus_id and mbr.month = cp.mill_date and mbr.source_type = 'internal' 
            where mm.status = 'active' and mm.source <> 'EB'and  mm.meter_type = 'primary' {where1} {where_internal}
            group by mp.campus_id,mm.source
            order by c.campus_id,mm.source
            '''
    createFolder("Log/", "availability report query3 " + str(query3))
    data3 = await cnx.execute(query3)
    data3 = data3.mappings().all()
    ...
    if len(data3) > 0:
        for row3 in data3:
            ...
            budget_mix = 0
            if report_method == 'final':
                actual = row3["actual_total"]
            else:
                actual = row3["actual"]
            
            budget_mix = row3["budget_mix"]
            budget = row3["budget"]
            kWh = row3["kWh"]
            # print("internal",actual*consumption) 
            campus_name = row3["campus_name"]
            if campus_name not in output3:
                output3[campus_name] = []
                internal_total_consumptions = 0
            internal_total_consumptions += kWh
            output3[campus_name].append({
                "consumption": kWh,
                "budget_amount": kWh * budget,
                "actual_amount": kWh * actual,
                "rate_variation": (kWh * budget) - (kWh * actual),
                "budget": budget,
                "actual": actual,
                "budget_mix": budget_mix,
                "internal_total_consumptions":internal_total_consumptions
            })
    overall_consumption_per_campus = {}
    for campus, records in output1.items():
        if records:
            external_consumption = records[-1]['external_total_consumptions']
            overall_consumption_per_campus[campus] = external_consumption

    for campus, records in output3.items():
        if records:
            internal_consumption = records[-1]['internal_total_consumptions']
            overall_consumption_per_campus[campus] = overall_consumption_per_campus.get(campus, 0) + internal_consumption

    for campus, records in output1.items():
        if records:
            overall_consumption = overall_consumption_per_campus.get(campus, 0)
            for entry in records:
                budget_mix = entry["budget_mix"]
                actual = entry["actual"]
                budget = entry["budget"]
                budget_amount = entry["budget_amount"]
                actual_amount = entry["actual_amount"]
                unit_mix = budget_mix / 100 * overall_consumption
                entry["unit_mix"] = unit_mix
                entry["mix_amount"] = unit_mix*budget
                entry["total_amount"] = (unit_mix*budget)-budget_amount
    ...
    campus_external_totals = {}
    for campus_name, campus_data in output1.items():
        total_budget_amount = 0
        total_actual_amount = 0
        total_rate_variation = 0
        total_budget_mix = 0
        ...
        for entry in campus_data:
            external_total_consumptions = entry["external_total_consumptions"]
            total_budget_amount += entry["budget_amount"]
            total_actual_amount += entry["actual_amount"]
            total_rate_variation += entry["rate_variation"]
            total_total_amount += entry["total_amount"]
            
        campus_external_totals[campus_name] = {
            "total_consumptions": external_total_consumptions,
            "total_budget_amount": total_budget_amount,
            "total_actual_amount": total_actual_amount,
            "total_rate_variation": total_rate_variation,
            "total_budget_mix": total_budget_mix,
            "total_total_amount": total_total_amount
        }
    
    campus_internal_totals = {}
    for campus_name, campus_data in output3.items():
        total_budget_amount = 0
        total_actual_amount = 0
        total_rate_variation = 0
        total_budget_mix = 0
        ...
        for entry in campus_data:
            internal_total_consumptions = entry["internal_total_consumptions"]
            total_budget_amount += entry["budget_amount"]
            total_actual_amount += entry["actual_amount"]
            total_rate_variation += entry["rate_variation"]
            total_total_amount += entry["total_amount"]
            
        campus_internal_totals[campus_name] = {
            "total_consumptions": internal_total_consumptions,
            "total_budget_amount": total_budget_amount,
            "total_actual_amount": total_actual_amount,
            "total_rate_variation": total_rate_variation,
            "total_budget_mix": total_budget_mix,
    ...
    # Combine external and internal totals
    for campus_name in set(output1.keys()).union(output3.keys()):
        external_totals = campus_external_totals.get(campus_name, {})
        internal_totals = campus_internal_totals.get(campus_name, {})

        total_consumptions = external_totals.get("total_consumptions", 0) + internal_totals.get("total_consumptions", 0)
        total_budget_amount = external_totals.get("total_budget_amount", 0) + internal_totals.get("total_budget_amount", 0)
        total_actual_amount = external_totals.get("total_actual_amount", 0) + internal_totals.get("total_actual_amount", 0)
        total_rate_variation = external_totals.get("total_rate_variation", 0) + internal_totals.get("total_rate_variation", 0)
        total_total_amount = external_totals.get("total_total_amount", 0) + internal_totals.get("total_total_amount", 0)
    
        campus_totals[campus_name] = {
    ...
    query2 = f'''
            select
                ...
                ROUND(SUM(CASE WHEN mm.meter_type = 'primary' and mbd.department_type = 'utility' THEN cp.equipment_kwh ELSE 0 END),prf.kWh) AS pm_common_kwh,  
                ROUND(SUM(CASE WHEN mm.meter_type = 'Primary' and mbd.department_type = 'overall' THEN cp.equipment_kwh ELSE 0 END),prf.kWh) AS pm_kwh,
                mbd.department_type as department_type,
                mbd.is_corporate,
                (
                        SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
                        FROM master_despatch_month d1
                        WHERE d1.ORGANIZATION_CODE = REPLACE(SUBSTRING(SUBSTRING_INDEX(mp.plant_code, '-', 3), LENGTH(SUBSTRING_INDEX(mp.plant_code, '-', 3 - 1)) + 1), '-', '')
                        AND d1.ITEM_CAT1 = mbe.reporting_department  and d1.DESPATCH_MONTH ='{year_month}'
                        AND d1.ITEM_CAT2 = 'Customer/WH'
                ) AS cust_wh_ton,
                (
                        SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
                        FROM master_despatch_month d1
            ...
            where mm.status = 'active' and mbe.status = 'active' and mbe.financial_year = '{year}' {where1}
            group by mp.campus_id,mbd.reporting_department_id
            order by c.campus_id,mbd.reporting_department_id
            '''
    data_2 = await cnx.execute(query2)
    data_2 = data_2.mappings().all()
    ...
    if len(data_2) > 0:
        ...
        for row3 in data_2:
            campus_id = row3["campus_id"]
            campus_name = row3["campus_name"]
            plant_name = row3["plant_name"]
            pm_common_kwh = row3["pm_common_kwh"]
            pm_kwh = row3["pm_kwh"]
            cust_wh_ton = row3["cust_wh_ton"]
            other_plant_ton = row3["other_plant_ton"]
            budget = row3["budget"]
            reporting_department = row3["reporting_department"]
            
            if campus_name != pre_campus_name and pre_campus_name!= '':
                total_consumptions = campus_totals[pre_campus_name]["total_consumptions"]
                total_budget_amount = campus_totals[pre_campus_name]["total_budget_amount"]
                units_variation = total_consumptions - total_pm_kwh-total_common_kwh
                
               
                    
                for row_4 in output_2[pre_campus_name]:
                    avg_rate_unit = 0
                    if total_pm_kwh != 0:
                        row_4["avg_rate_unit"] =  round(total_budget_amount / total_consumptions,2)
                        avg_rate_unit =  round(total_budget_amount / total_consumptions,2)
                        row_4['pm_common_kwh'] = row_4["pm_kwh"] / total_pm_kwh * total_common_kwh
                        row_4['unit_variation'] = row_4["pm_kwh"] / total_pm_kwh * units_variation
                        
                    else:
                        row_4['pm_common_kwh'] = 0
                        row_4['unit_variation'] = 0
                        row_4["avg_rate_unit"] = 0

                    row_4['total_unit'] = row_4["pm_kwh"] + row_4["pm_common_kwh"]  + row_4["unit_variation"]
                    if row_4["total_ton"] != 0:
                        row_4['actual'] = (row_4['total_unit'] / round(row_4["total_ton"]))
                    else:
                        row_4['actual'] = 0
                       
                    row_4['u/t'] = row_4["budget"] - row_4['actual']
                    if pre_campus_name == 'TPI Avadi':
                        print("u/t",row_4['u/t'])
                        print("budget",row_4['budget'])
                        print("actual",row_4["actual"])
                        print("total_unit:", row_4["total_unit"])  
                        print("total_ton:", row_4["total_ton"])  
                    
                    row_4["specific_consumption_variation"] =  row_4["total_ton"]*(row_4["budget"] - row_4['actual'])*avg_rate_unit
            pre_campus_name = campus_name

            if campus_name not in output_2:
                output_2[campus_name] = []
                total_pm_kwh = 0
                total_common_kwh = 0  
                current_plant_name = ''
            
            if current_plant_name == '' or current_plant_name != plant_name:
                total_common_kwh += pm_common_kwh
            
            total_pm_kwh += pm_kwh
            current_plant_name = plant_name
            output_2[campus_name].append({
                "pm_common_kwh": (pm_common_kwh),
                "pm_kwh": round(pm_kwh),
                "reporting_department": reporting_department,
                "cust_wh_ton": cust_wh_ton,
                "other_plant_ton": other_plant_ton,
                "total_ton": round(cust_wh_ton+other_plant_ton),
                "budget": budget,
                "unit_variation": 0,
                "total_unit": 0,
                "actual": 0,
        ...
        if pre_campus_name!= '':
            ...
            for row_4 in output_2[pre_campus_name]:
                ...
                row_4['u/t'] = row_4["budget"] - row_4['actual']
                row_4["specific_consumption_variation"] =  (row_4["total_ton"])*(row_4['u/t'])*(row_4["avg_rate_unit"])
        
        for campus_name, campus_data in output_2.items():
            specific_consumption_variation = 0

            for entry in campus_data:
                specific_consumption_va = entry["specific_consumption_variation"]
                specific_consumption_variation += entry["specific_consumption_variation"]
                print("specific_consumption_variation",campus_name)
                print("specific_consumption_variation",specific_consumption_va)
                print("specific_consumption_variation",specific_consumption_variation)

            campus_totals[campus_name]["specific_consumption_variation"] = specific_consumption_variation

    return campus_totals

async def fin_year(date):
    
    start_of_financial_year = datetime(date.year if date.month >= 4 else date.year - 1, 4, 1)

    dates_list = []
    
    current_date = start_of_financial_year
    current_month= datetime.now()
    current_month = datetime.strftime(current_month,'%m')
    while current_date <=date and current_month!= current_date.month:
        from_date = await parse_date(current_date.strftime("%d-%m-%Y"))
        to_date = from_date.replace(day=1, month=from_date.month + 1)
        to_date = to_date - timedelta(days=1)
...
#             external_total_consumptions +=total_consumption
#             if energy_source_name != 'EB':
#                 # print("external",actual*consumption)  
#                 overall_total += consumption
...
#                 if campus_name in output1  and  len(output1[campus_name]) > 0:
#                     try:
#                         # print("external_eb",output1[campus_name][-1]["overall_total"])  
#                         # print("external_eb",row["consumption"])  
#                         # print("actual_eb",row["actual"])  
#                         # print("external_eb",(row["consumption"] - output1[campus_name][-1]["overall_total"]) * actual)  
...
#     print("Overall Consumption per Campus:")
#     # for campus, consumption in overall_consumption_per_campus.items():
#     for campus, records in output1.items():
#         if records:
#             overall_consumption = overall_consumption_per_campus.get(campus, 0)
#             for entry in records:
...
#             total_rate_variation += entry["rate_variation"]
#             total_total_amount += entry["total_amount"]
#             # print("total_actual_amount",total_actual_amount)
...
#             total_rate_variation += entry["rate_variation"]
#             total_total_amount += entry["total_amount"]
#             # print("inttotal_actual_amount",total_actual_amount)
#         campus_internal_totals[campus_name] = {
#             "total_consumptions": internal_total_consumptions,
#             "total_budget_amount": total_budget_amount,
#             "total_actual_amount": total_actual_amount,
...
#                     row_4["specific_consumption_variation"] =  row_4["total_ton"]*(row_4["budget"] - row_4['actual'])*avg_rate_unit
#                     if pre_campus_name == 'TPI Avadi':
#                         # print("u/t",round(row_4['u/t']))
#                         # print("pm_common_kwh",round(row_4['pm_common_kwh']))
#                         # print("pm_kwh",round(row_4["pm_kwh"]))
#                         # print("avg:", row_4["avg_rate_unit"])  
#                         # print("unit_variation:", row_4["unit_variation"])  
#                     #     print("total_budget_amount:", total_budget_amount)  
#                     #     print("total_pm_kwh:", total_pm_kwh)  
#                     #     print("campus_name:", pre_campus_name)  
#                     #     print("total_common_kwh:", total_common_kwh)  
#                         # print("ton:", row_4["total_ton"])  
#                         # print("actual:", row_4["actual"])  
#                         # print("budget:", row_4["budget"])  
#                         print("specific_consumption_variation:", row_4["specific_consumption_variation"])  
...
#             total_pm_kwh += pm_kwh
#             # if campus_name == 'TPI Avadi':
#                 # print("1111111111",total_pm_kwh)
#             current_plant_name = plant_name
...
#             FROM 
#                 master_source_entry mse
#                 left JOIN master_source_entry_date msed ON msed.energy_source_id = mse.id {where_year}
#                 left JOIN master_budget_rate mbr ON mbr.energy_source_name = mse.energy_source_name 
#                 and mbr.campus_id = mse.campus_id and mbr.month = msed.mill_date and mbr.source_type = 'external'
#                 INNER JOIN  master_campus c ON c.campus_id = mse.campus_id
#             where 1=1  and mse.status ='active' and mse.source_type = 'external'{where_external}
...
#             order by c.campus_id,mse.id
#             '''
#     createFolder("Log/", "query_year " + str(query_year))
#     data_year = await cnx.execute(query_year)
#     data_year = data_year.mappings().all()
#     output1_year = {}
...
#             campus_name = row["campus_name"]
#             if campus_name not in output1_year:
#                 output1_year[campus_name] = []
#                 external_total_consumptions = 0
#                 overall_total = 0
#             external_total_consumptions +=total_consumption
#             if energy_source_name != 'EB':
...
#                 overall_total += consumption
#                 output1_year[campus_name].append({
#                     "consumption": consumption,
#                     "budget_amount": consumption * budget,
#                     "actual_amount": consumption * actual,
#                     "rate_variation": (consumption * budget) - (consumption * actual),
...
#             campus_name = row["campus_name"]
#             # print(campus_name)
#             # print(output1_year[campus_name])
#             if energy_source_name == 'EB':
#                 if campus_name in output1_year  and  len(output1_year[campus_name]) > 0:
#                     try:

#                         output1_year[campus_name].append({
#                             "consumption": consumption - output1_year[campus_name][-1]["overall_total"],
#                             "budget_amount": (consumption - output1_year[campus_name][-1]["overall_total"]) * budget,
#                             "actual_amount": (consumption - output1_year[campus_name][-1]["overall_total"]) * actual,
#                             "rate_variation": ((consumption - output1_year[campus_name][-1]["overall_total"]) * budget) - ((consumption - output1_year[campus_name][-1]["overall_total"]) * actual),
#                             "budget": budget,
#                             "actual": actual,
#                             "budget_mix": budget_mix,
#                             "external_total_consumptions": total_consumption
...
#                 left JOIN master_budget_rate mbr ON mbr.energy_source_name = mm.source 
#                 and mbr.campus_id = mp.campus_id and mbr.month = cp.mill_date and mbr.source_type = 'internal' 
#             where mm.status = 'active' and mm.source <> 'EB'and  mm.meter_type = 'primary' {where_year_internal} {where_internal}
#             group by mp.campus_id,mm.source
#             order by c.campus_id,mm.source
#             '''
#     createFolder("Log/", "availability report query3 " + str(query_year2))
#     data_year2 = await cnx.execute(query_year2)
#     data_year2 = data_year2.mappings().all()
#     output2_year = {}
#     if len(data_year2) > 0:
#         for row_year in data_year2:
#             if report_method == 'final':
#                 actual = row_year["actual_total"]
#             else:
#                 actual = row_year["actual"]
            
#             budget_mix = row_year["budget_mix"]
#             budget = row_year["budget"]
#             kWh = row_year["kWh"]

#             campus_name = row_year["campus_name"]
#             if campus_name not in output2_year:
#                 output2_year[campus_name] = []
#                 internal_total_consumptions = 0
#             internal_total_consumptions +=kWh
#             output2_year[campus_name].append({
#                 "consumption": kWh,
#                 "budget_amount": kWh * budget,
#                 "actual_amount": kWh * actual,
#                 "rate_variation": (kWh * budget) - (kWh * actual),
...
#     for campus, records in output1_year.items():
#         if records:
#             external_consumption = records[-1]['external_total_consumptions']
#             overall_consumption_per_campus[campus] = external_consumption

#     for campus, records in output2_year.items():
#         if records:
#             internal_consumption = records[-1]['internal_total_consumptions']
#             overall_consumption_per_campus[campus] = overall_consumption_per_campus.get(campus, 0) + internal_consumption
...
#                 unit_mix = budget_mix / 100 * overall_consumption
#                 entry["unit_mix"] = unit_mix
#                 entry["mix_amount"] = unit_mix*actual
#                 entry["total_amount"] = (unit_mix*actual)-actual_amount

    
#     campus_external_totals = {}

#     for campus_name, campus_data in output1_year.items():
#         total_budget_amount = 0
#         total_actual_amount = 0
#         total_rate_variation = 0
#         total_budget_mix = 0
...
#     for campus_name, campus_data in output2_year.items():
#         total_budget_amount = 0
#         total_actual_amount = 0
#         total_rate_variation = 0
#         total_budget_mix = 0
...
#     # Combine external and internal totals
#     for campus_name in set(output1_year.keys()).union(output2_year.keys()):
#         external_totals = campus_external_totals.get(campus_name, {})
#         internal_totals = campus_internal_totals.get(campus_name, {})
...
#         # Combine other totals as needed

#         # Store the combined totals for the current campus
#         campus_totals_year[campus_name] = {
#             "total_consumptions": total_consumptions,
#             "total_budget_amount": total_budget_amount,
#             "total_actual_amount": total_actual_amount,
#             "total_rate_variation": total_rate_variation,
...
#             "specific_consumption_variation":0
#         }
#         # print("campus_totals_year",campus_totals_year)
#     query2_year = f'''
#             select
#                 mbe.*,
#                 mp.plant_name,
...
#                         FROM master_despatch_month d1
#                         WHERE d1.ORGANIZATION_CODE = REPLACE(SUBSTRING(SUBSTRING_INDEX(mp.plant_code, '-', 3), LENGTH(SUBSTRING_INDEX(mp.plant_code, '-', 3 - 1)) + 1), '-', '')
#                         AND d1.ITEM_CAT1 = mbe.reporting_department  and d1.DESPATCH_MONTH in ('{year_month_all}')
#                         AND d1.ITEM_CAT2 = 'Customer/WH'
#                 ) AS cust_wh_ton,
#                 (
#                         SELECT ifnull(SUM(d1.INVOICE_QTY_TON),0)
...
#             order by c.campus_id,mbd.reporting_department_id
#             '''
#     data_2_year = await cnx.execute(query2_year)
#     data_2_year = data_2_year.mappings().all()
...
#             if campus_name != pre_campus_name and pre_campus_name!= '':
#                 total_consumptions = campus_totals_year[pre_campus_name]["total_consumptions"]
#                 total_budget_amount = campus_totals_year[pre_campus_name]["total_budget_amount"]
#                 units_variation = total_consumptions -total_pm_kwh-total_common_kwh
                    
#                 for row_4 in output_2_year[pre_campus_name]:
#                     if total_pm_kwh != 0:
#                         row_4['pm_common_kwh'] = (row_4["pm_kwh"] / total_pm_kwh) * total_common_kwh
#                         row_4['unit_variation'] = (row_4["pm_kwh"] / total_pm_kwh) * units_variation
#                         row_4["avg_rate_unit"] =  round(total_budget_amount/total_consumptions,2)
...
#                     row_4['total_unit'] = row_4["pm_kwh"] +row_4["pm_common_kwh"]+row_4["unit_variation"]
#                     if row_4["total_ton"] != 0:
#                         row_4['actual'] = (row_4["pm_kwh"] +row_4["pm_common_kwh"]+row_4["unit_variation"])/row_4["total_ton"]
#                         row_4['u/t'] = row_4["budget"] - ((row_4["pm_kwh"] +row_4["pm_common_kwh"]+row_4["unit_variation"])/row_4["total_ton"])
#                     else:
#                         row_4['actual'] = 0
#                         row_4['u/t'] = row_4["budget"]
                    
#                     row_4["specific_consumption_variation"] =  round(row_4["total_ton"])*row_4['u/t']*row_4["avg_rate_unit"]
            
#             pre_campus_name = campus_name

#             if campus_name not in output_2_year:
#                 output_2_year[campus_name] = []
#                 total_pm_kwh = 0
#                 total_common_kwh = 0  
#                 current_plant_name = ''
...
#             output_2_year[campus_name].append({
#                 "pm_common_kwh": pm_common_kwh,
#                 "pm_kwh": round(pm_kwh),
#                 "reporting_department": reporting_department,
#                 "cust_wh_ton": cust_wh_ton,
...
#         for campus_name, campus_data in output_2_year.items():
#             specific_consumption_variation = 0

#             for entry in campus_data:
#                 specific_consumption_variation += entry["specific_consumption_variation"]
#             campus_totals_year[campus_name]["specific_consumption_variation"] = specific_consumption_variation


        
#         fin_date = await fin_year((fin_from_date))
#         fin_year_report = {}
        
#         for data in fin_date:
#             from_date = datetime.strptime(data['startDate'],'%d-%m-%Y')
#             to_date = datetime.strptime(data['endDate'],'%d-%m-%Y')
#             result_data = await FinancialYearReport(cnx,campus_id,report_method,from_date,to_date,from_year,to_year)
            
#             for campusname, campusdata in result_data.items():
#                 if campusname not in fin_year_report:
#                     fin_year_report[campusname] = {
#                         "total_consumptions": 0,
#                         "total_budget_amount": 0,
#                         "total_actual_amount": 0,
#                         "total_rate_variation": 0,
...
#                 fin_year_report[campusname]['total_consumptions'] += campusdata['total_consumptions']
#                 fin_year_report[campusname]['total_budget_amount'] += campusdata['total_budget_amount']
#                 fin_year_report[campusname]['total_actual_amount'] += campusdata['total_actual_amount']
#                 fin_year_report[campusname]['total_rate_variation'] += campusdata['total_rate_variation']
#                 fin_year_report[campusname]['total_total_amount'] += campusdata['total_total_amount']
#                 fin_year_report[campusname]['specific_consumption_variation'] += campusdata['specific_consumption_variation']
        
        
#         campus_totals_year=fin_year_report
#     result = {"data":campus_totals,"data2":campus_totals_year}
#     return result

#  kavin
# to get month and  financial year  report
...
# async def campuswisepowerconsumptionunits(cnx, campus_id, report_method, from_date, to_date, from_year, to_year):
    try:
        ...
        result_data = await FinancialYearReport(cnx, fin_campus_id, report_method, from_date, to_date, from_year, to_year)
        ...
        # Fetch data for the financial year
        fin_date = await fin_year(fin_from_date)
        fin_year_data = {}
...
#         table_name_year = f"( {subquery_union_year}) cp"
#         year_month_all = ",".join(year_months)
#     print("union_query_year",union_query_year)
#     where_internal = ''
#     where_external = ''
