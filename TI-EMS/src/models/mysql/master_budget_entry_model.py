...
async def savebudget(cnx,campus_id,company_id,bu_id,plant_id,reporting_department, department_ids,utility_department_ids,is_corporate,financial_year,budget,user_login_id):
    ...
    sql = f'''insert into master_budget_entry(campus_id,company_id,bu_id,plant_id,reporting_department,department_ids,utility_department_ids,financial_year,budget,created_on,created_by)
    ...
    if source_type != '':
        ...
        query = f'''
                ...
                FROM
                    ...
                    left join master_budget_rate mbr on mbr.energy_source_name = mse.energy_source_name and mbr.campus_id = mse.campus_id and mbr.source_type = '{source_type}' {on_con}
                ...
                where  mse.status = 'active' and mse.source_type = '{source_type}' {where}  '''
        data = await cnx.execute(query)
        data = data.fetchall()
        print("budget_rate_query : ", query)
    # else:
    #     query = f'''
    #             select
    #             IFNULL(mbr.id, '') AS id, 
...
async def savebudget_rate(cnx,campus_id,obj,month,source_type,user_login_id,is_year_wise):
    ...
    for row in data:
        ...
        if id == '':
            if source_type == 'fixed_demand':
                budget_demand = row["budget_demand"]
            else:                
                budget_mix = row["budget_mix"]
            for i in range(3,12):
                date = from_date.replace(day=1, month=from_date.month + i,year = from_date.year-1)
                query = f'''insert into master_budget_rate(campus_id,energy_source_name,month,budget,source_type,created_on,created_by,budget_mix,budget_demand)
                            values('{campus_id}','{energy_source_name}','{date}','{budget}','{source_type}',now(),'{user_login_id}','{budget_mix}','{budget_demand}')'''
                await cnx.execute(text(query))
                await cnx.commit()
            for i in range(0,3):
            ...
            if is_year_wise == 'yes':
                if source_type == 'fixed_demand':
                    ...
                    budget_mix = row["budget_mix"]
                start_date = from_date.replace(day=1, month=4,year = from_date.year-1)
                to_date = from_date.replace(day=1, month=3,year = from_date.year)
                ...
                query = f'''update master_budget_rate set 
                            ...
                            modified_on = now(),
                            modified_by = {user_login_id},
                            budget_demand = '{budget_demand}'
                            where  month >= '{start_date}' and month <= '{to_date}' and energy_source_name = '{energy_source_name}' and source_type = '{source_type}' and campus_id = {campus_id} '''
                await cnx.execute(text(query))
                await cnx.commit()
                ...
                if source_type == 'fixed_demand':
                    ...
                    actual_demand_final = row["actual_demand_final"]
                actual = row["actual"]
                ...
                query = f'''update master_budget_rate set 
                            ...
                            modified_by = {user_login_id},
                            budget_demand = '{budget_demand}',
                            actual_demand_final = '{actual_demand_final}',
