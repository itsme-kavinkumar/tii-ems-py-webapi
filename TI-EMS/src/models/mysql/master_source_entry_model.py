...
async def source_entry_list(cnx, id,period_type,source_type):
    ...
    query = text(f"""
        SELECT                
            s.*,
            s.energy_source_name as updated_energy_source_name,
            c.campus_name,
...
async def update_source_entry(cnx, id,campus_id,period_type,energy_source_name,user_login_id,source_type,updated_energy_source_name):
    ...
    query =text(f"""
        ...
        SET 
            ...
            energy_source_name = '{updated_energy_source_name}', 
            modified_on = NOW(),
    ...
    query1 =text(f"""
        UPDATE 
            ems_v1.master_budget_rate
        SET 
            energy_source_name = '{updated_energy_source_name}'            
        WHERE energy_source_name = '{energy_source_name}'  and campus_id = '{campus_id}'
    """)
...
async def save_source_entry_data(cnx, obj,obj2,mill_date_month,campus_id,mill_date,user_login_id):
    ...
    start_date = datetime.strptime(mill_date_month, "%d-%m-%Y").strftime("%Y-%m-%d 00:00:00")
    end_date = (datetime.strptime(mill_date_month, "%d-%m-%Y") + relativedelta(months=1) - timedelta(days=1)).strftime("%Y-%m-%d 00:00:00")
    ...
    if obj_month:
        query = '''
                SELECT 
	                SUM( msd.consumption) AS consumption , SUM( msd.consumption_total) AS consumption_total
                FROM 
	                master_source_entry_date msd
                INNER JOIN  master_source_entry me ON me.id =  msd.energy_source_id
                WHERE  
	                msd.campus_id = :campus_id AND me.energy_source_name = 'EB' AND me.period_type = 'date' AND  msd.mill_date BETWEEN :start_date AND :end_date
                '''
        values = {
            'campus_id':campus_id,
        ...
        result = await cnx.execute(query, values)
        result = result.mappings().all()
       
        obj_consumption, obj_consumption_total = reduce(
            lambda acc, d: (acc[0] + int(d['consumption']), acc[1] + int(d['consumption_total'])),
            obj_month,
            (0, 0)
        )
       
        if (obj_consumption <= int(result[0]['consumption']) and obj_consumption_total<= int(result[0]['consumption_total'])):
