...
async def load_analysis_mdl(cnx,company_id,bu_id,plant_id,period_id,group_by,equipment_id,meter_id,from_date,to_date,shift_id,from_time,to_time,duration,meter_status, detail_view):
        ...
        query=text(f'''
            ...
            FROM (
                ...
                ROUND(SUM(case when mmf.kWh = '*' then cp.kWh * mmf.kWh_value  when mmf.kWh = '/' then cp.kWh / mmf.kWh_value else cp.kWh end),{ 2 if detail_view == 'yes' else 'prf.kWh'}) AS kWh,
                ...
                ROUND(SUM(CASE WHEN mm.meter = 'equipment' and mm.is_poll_meter = 'yes'THEN  cp.diff_equipment_on_load_kwh ELSE 0 END ),{ 2 if detail_view == 'yes' else 'prf.kWh'}) as on_load_kwh,
                ROUND(SUM(CASE WHEN mm.meter = 'equipment' and mm.is_poll_meter = 'yes'THEN  cp.diff_equipment_off_kwh  ELSE 0 END ),{ 2 if detail_view == 'yes' else 'prf.kWh'}) as off_kwh,
                ROUND(SUM(CASE WHEN mm.meter = 'equipment' and mm.is_poll_meter = 'yes'THEN  cp.diff_equipment_idle_kwh ELSE 0 END ),{ 2 if detail_view == 'yes' else 'prf.kWh'}) as idle_kwh,
