...
async def gateway_log(cnx,mac,from_date,from_time,to_time):
    ...
    if from_date != '' and from_date[:11] == dates.strftime('%Y-%m-%d'):
...
async def availability_report(cnx,campus_id,from_date,to_date,year,report_type):
    ...
    if from_date != '':
        ...
        where_d += f" and d1.DESPATCH_DATE >='{from_date}'"
    ...
    if to_date != '':
        ...
        where_d += f" and d1.DESPATCH_DATE <='{to_date}'"
    ...
    query2 = f'''
            select
                ...
                (
                    ...
                    AND d1.ITEM_CAT1 = mbe.reporting_department {where_d} 
...
async def stagewisekwhvariation_report(cnx,campus_id,from_date,to_date):
    ...
    sql = f'''select 
            ...
            from 
                ...
                inner JOIN ems_v1.master_meter_factor mmf ON  mmf.plant_id = mp.plant_id AND mmf.meter_id = mm.meter_id
                inner JOIN ems_v1.master_parameter_roundoff prf ON prf.plant_id = mp.plant_id 
            where mm.status = 'active' and cp.mill_date >='{from_date}' and cp.mill_date <='{to_date}' and mm.meter_type = 'primary' {where}
    ...
    if len(data1)>0:
            ...
            for demand_meter_data in main_demand_meter_data:
                ...
                if len(formula2)>0:
                    ...
                    for trans_m in data2:
                        ...
                        query2 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{main_transformer_meter_id}]%'  and status = 'active'"
        ...
        if final_main_transformer_data != []:
            for transformer_d in final_main_transformer_data:
                ...
                formula = transformer_d["formula2"]
                meters = re.findall(r'\(([^)]+)\)', formula)
                if meters :
                    formula = f"({','.join(meters)})"
                    meters = re.findall(r'\[(\d+)\]', formula)
                ...
                if len(formula2)>0:
                    ...
                    if len(data3)>0:
                        ...
                        for meter_id in formula2.split(','):
                            query3 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{meter_id}]%'  and status = 'active'"
                            ...
                            if len(formula_data3)>0:
                                ...
                                if meters :
                                    ...
                                    meters = meters[1:]  
                                    all_meters = f"{','.join(meters)}"
                                print("1234533246",all_meters)
                            if all_meters != []:
                                all_meters1 +=  all_meters + ","
                                print("all_meters1",all_meters1)
                        if len(all_meters1)>0:
                            sql4 = f'''
                                select 
                                    mm.meter_id,
                                    mm.meter_name,
                            ...
                            if all_meters1 != []:
                                for  parent_meter_id in all_meters1.split(','):
                                    query4 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{parent_meter_id}]%'  and status = 'active'"
                        ...
                        if all_parent_meters1!= []:
                            for  child_meter_id in all_parent_meters1.split(','):
                                query5 = f"SELECT * FROM master_equipment_calculations WHERE formula2 LIKE ' dict[{child_meter_id}]%'  and status = 'active'"
        ...
        return {"data":data,"data1":main_demand_meter_data,"data2":final_main_transformer_data,"data3":final_sub_transformer_data,"data4":parent_meter_data,"data5":child_meter_data,"data6":child_meter_data1}
    else:
        return data
...
async def campuswisepowerconsumptionunits(cnx,campus_id,report_method,from_date,to_date,from_year,to_year):
    ...
    if len(data)>0:
        for row in data:
            ...
            campus_name = row["campus_name"]
            print(campus_name)
            print(output1[campus_name])
            if energy_source_name == 'EB':
                if campus_name in output1  and  len(output1[campus_name]) > 0:
                    try:

                        output1[campus_name].append({
                            "consumption": row["consumption"] - output1[campus_name][-1]["overall_total"],
                            "budget_amount": (row["consumption"] - output1[campus_name][-1]["overall_total"]) * row["budget"],
                            "actual_amount": (row["consumption"] - output1[campus_name][-1]["overall_total"]) * row["actual"],
                            "rate_variation": ((row["consumption"] - output1[campus_name][-1]["overall_total"]) * row["budget"]) - ((row["consumption"] - output1[campus_name][-1]["overall_total"]) * row["actual"]),
                            "budget": row["budget"],
                            "actual": row["actual"],
                            "budget_mix": row["budget_mix"],
                            "external_total_consumptions": row["provision_total_consumption"]
                        })
    ...
    query2 = f'''
            ...
            order by mse.campus_id,mse.id
            '''
    createFolder("Log/", "availability report query " + str(query2))
    data2 = await cnx.execute(query2)
    data2 = data2.mappings().all()
    output2 = {}
    external_total_consumptions = 0
    overall_consumption_year = 0
    if len(data2) > 0:
        for row2 in data2:
            if report_method == 'final':
                consumption = row2["consumption_total"]
                total_consumption = row2["final_total_consumption"]
                actual = row2["actual_total"]
            else:
                consumption = row2["consumption"]
                total_consumption = row2["provision_total_consumption"]
                actual = row2["actual"]
                
            budget_mix = row2["budget_mix"]
            budget = row2["budget"]
            energy_source_name= row2["energy_source_name"]

            campus_name = row2["campus_name"]
            if campus_name not in output2:
                output2[campus_name] = []
                external_total_consumptions = 0
                overall_consumption_year = 0
            external_total_consumptions +=total_consumption
            if energy_source_name !='EB':
                overall_consumption_year +=consumption
                output2[campus_name].append({
                    "consumption": consumption,
                    "budget_amount": consumption * budget,
                    "actual_amount": consumption * actual,
                    "rate_variation": (consumption * budget) - (consumption * actual),
                    ...
                    "budget_mix": budget_mix,
                    "external_total_consumptions":external_total_consumptions,
                    "overall_consumption_year":overall_consumption_year
    ...
    if len(data2)>0:
        for row in data2:
            ...
            campus_name = row["campus_name"]
            if energy_source_name == 'EB':
                if campus_name in output2 and  len(output2[campus_name]) > 0:
                    try:
                        output2[campus_name].append({
                            "consumption": row["consumption"] - output2[campus_name][-1]["overall_consumption_year"],
                            "budget_amount": (row["consumption"] - output2[campus_name][-1]["overall_consumption_year"]) * row["budget"],
                            "actual_amount": (row["consumption"] - output2[campus_name][-1]["overall_consumption_year"]) * row["actual"],
                            "rate_variation": ((row["consumption"] - output2[campus_name][-1]["overall_consumption_year"]) * row["budget"]) - ((row["consumption"] - output2[campus_name][-1]["overall_consumption_year"]) * row["actual"]),
                            "budget": row["budget"],
                            "actual": row["actual"],
                            "budget_mix": row["budget_mix"],
                            "external_total_consumptions": row["provision_total_consumption"]
    ...
    if table_name_year!= '':
        query4 = f'''
            ...
            order by mm.plant_id,mm.source
            '''
        createFolder("Log/", "availability report query3 " + str(query4))
        data4 = await cnx.execute(query4)
        data4 = data4.mappings().all()
        output4 = {}
        if len(data4) > 0:
            for row4 in data3:
                if report_method == 'provision':
                    actual = row4["actual"]
                elif report_method == 'final':
                    actual = row4["actual_total"]
                budget_mix = row4["budget_mix"]
                budget = row4["budget"]
                kWh = row4["kWh"]

                campus_name = row4["campus_name"]
                if campus_name not in output4:
                    output4[campus_name] = []
                    internal_total_consumptions = 0
                internal_total_consumptions +=kWh
                output4[campus_name].append({
                    "consumption": kWh,
                    "budget_amount": kWh * budget,
                    "actual_amount": kWh * actual,
                    "rate_variation": (kWh * budget) - (kWh * actual),
        ...
        for campus, records in output2.items():
            if records:
                external_consumption = records[-1]['external_total_consumptions']
                overall_consumption_per_campus[campus] = external_consumption

        for campus, records in output4.items():
            if records:
                internal_consumption = records[-1]['internal_total_consumptions']
                overall_consumption_per_campus[campus] = overall_consumption_per_campus.get(campus, 0) + internal_consumption
        ...
        campus_external_total_year = {}

        for campus_name, campus_data in output2.items():
            total_budget_amount = 0
            total_actual_amount = 0
            total_rate_variation = 0
            total_budget_mix = 0
            ...
            campus_external_total_year[campus_name] = {
                "total_consumptions": external_total_consumptions,
                "total_budget_amount": total_budget_amount,
                "total_actual_amount": total_actual_amount,
                "total_rate_variation": total_rate_variation,
        ...
        campus_internal_total_year = {}

        for campus_name, campus_data in output4.items():
            total_budget_amount = 0
            total_actual_amount = 0
            total_rate_variation = 0
            total_budget_mix = 0
            ...
            campus_internal_total_year[campus_name] = {
                "total_consumptions": internal_total_consumptions,
                "total_budget_amount": total_budget_amount,
                "total_actual_amount": total_actual_amount,
                "total_rate_variation": total_rate_variation,
        ...
        # Combine external and internal totals
        for campus_name in set(output2.keys()).union(output4.keys()):
            external_totals = campus_external_total_year.get(campus_name, {})
            internal_totals = campus_internal_total_year.get(campus_name, {})
    ...
    result = {"data":campus_totals,"data2":campus_total_year}
    return result
