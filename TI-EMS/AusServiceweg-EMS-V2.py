...
class PythonWindowsService(win32serviceutil.ServiceFramework):
    ...
    def SvcDoRun(self):
        ...
        server_path = os.path.join(sys._MEIPASS, "AusDataProvian-EMS-V2\\AusDataProvian-EMS-V2.exe")
        self.process = Popen(server_path, stdout=PIPE, stderr=PIPE)
        stdout, stderr = self.process.communicate()
        
        if self.process.returncode != 0:
            error_message = f"AusDataProvian-EMS-V2.exe failed to run. Error: {stderr.decode('utf-8').strip()}"
